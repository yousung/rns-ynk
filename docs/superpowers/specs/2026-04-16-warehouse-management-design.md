...
### 3.3 권한 없는 창고 상품 딤처리 (UI 정책)

입·출고 예정 목록 및 재고 리스트에서 상품을 표시할 때, **해당 상품이 적재된 창고에 대한 접근 권한이 없으면** 다음 규칙을 적용한다.

| 상황 | 처리 방식 |
|---|---|
| 사용자가 해당 창고의 view 권한 없음 | 상품 항목을 딤처리(opacity 감소 + pointer-events: none) |
| 딤처리 항목에 마우스 오버 | 툴팁: `"적재 창고(OOO)에 대한 접근 권한이 없습니다"` |
| 사용자가 해당 창고의 view 권한 있음 | 정상 표시 및 클릭 가능 |

**적용 범위:**
- 입고 예정 목록 (`inbound-schedule`) — 예정 상품 카드
- 출고 예정 목록 (`outbound-schedule`) — 예정 상품 카드
- 입고 처리 (`inbound-execute`) — 예정 목록 사이드 패널
- 출고 처리 (`outbound-execute`) — 예정 목록 사이드 패널
- 재고 리스트 창고 뷰 (`inventory` warehouse 탭) — 상품 목록 항목

**자동 창고 전환 (출고 및 재고 리스트 한정):**
출고 예정 상품 클릭 시, 또는 재고 리스트 창고 뷰에서 상품 클릭 시, 현재 선택 창고에 해당 상품 재고가 없고 권한 있는 다른 창고에 재고가 있으면 해당 창고 탭으로 자동 이동한다.

- **입고 처리에서는 자동 창고 전환 없음.** 입고는 어느 창고에 넣을지 사용자가 직접 선택하는 행위이므로 자동 이동하지 않는다.

> **데모 범위 제외:** 현재 데모에서는 딤처리/툴팁은 구현하지 않으며 기획 정책으로만 기술한다. 자동 창고 전환(출고·재고 리스트)은 데모에 구현되어 있다.
