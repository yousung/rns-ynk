...
function switchWarehouseType(type, menu) {
  ...
  if (menu === 'inbound-execute') {
    window.location.href = type === 'a' ? '../type-a/inbound-execute.html' : 'inbound-execute.html';
  } else if (menu === 'outbound-execute') {
    window.location.href = type === 'a' ? '../type-a/outbound-execute.html' : 'outbound-execute.html';
  } else {
    renderLayout(menu);
...
function renderLayout(activeMenu) {
  ...
  const menuItems = [
    ...
    { key: 'inbound-execute',   label: '입고 처리',   href: getWarehouseType() === 'a' ? '../type-a/inbound-execute.html' : 'inbound-execute.html' },
    { key: 'outbound-schedule', label: '출고 예정',   href: '../../outbound-schedule.html' },
    { key: 'outbound-execute',  label: '출고 처리',   href: getWarehouseType() === 'a' ? '../type-a/outbound-execute.html' : 'outbound-execute.html' },
