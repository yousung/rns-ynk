...
입고/출고는 예정 등록 후 실행되는 구조이므로, 예정(`*_schedules`)과 실행 이력(`*_records`)을 분리한다. 출고는 `received_at` 오름차순 FIFO 기준으로 `inventory_items`를 차감한다.

재고 현재값은 `inventory_items.quantity`에 두고, 감사/복구/추적 기준은 `stock_movements` 원장을 사용한다. 키오스크/전동랙 상태는 장비 이벤트로 별도 저장한다.
...
| 입고 | `inbound_records` | 입고 실행 이력 |
| 출고 | `outbound_schedules` | 출고 예정 |
| 출고 | `outbound_records` | 출고 실행 헤더 |
| 출고 | `outbound_record_items` | FIFO 차감 상세 |
| 이력 | `stock_movements` | 입고/출고/이동/조정 재고 원장 |
| 사용자 | `users` | 계정 |
...
erDiagram
  ...
  inbound_schedules ||--o{ inbound_records : executed_by
  outbound_schedules ||--o{ outbound_records : executed_by
  outbound_records ||--o{ outbound_record_items : has
  inventory_items ||--o{ outbound_record_items : deducted_from
...
create table inbound_records (
  id bigint unsigned not null auto_increment,
  inbound_schedule_id bigint unsigned not null,
  ...
  primary key (id),
  key idx_inbound_records_schedule (inbound_schedule_id),
  key idx_inbound_records_processed_at (processed_at),
  constraint fk_inbound_records_schedule foreign key (inbound_schedule_id) references inbound_schedules(id),
  constraint fk_inbound_records_inventory_item foreign key (inventory_item_id) references inventory_items(id),
  constraint fk_inbound_records_pallet foreign key (pallet_id) references pallets(id),
  constraint fk_inbound_records_rack_slot foreign key (rack_slot_id) references rack_slots(id),
  constraint fk_inbound_records_processed_by foreign key (processed_by) references users(id),
  constraint chk_inbound_records_quantity check (quantity > 0)
) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci;
...
create table outbound_records (
  id bigint unsigned not null auto_increment,
  outbound_schedule_id bigint unsigned not null,
  product_id bigint unsigned not null,
  total_quantity decimal(14,3) not null,
  processed_by bigint unsigned null,
  processed_at datetime(3) not null default current_timestamp(3),
  primary key (id),
  key idx_outbound_records_schedule (outbound_schedule_id),
  key idx_outbound_records_processed_at (processed_at),
  constraint fk_outbound_records_schedule foreign key (outbound_schedule_id) references outbound_schedules(id),
  constraint fk_outbound_records_product foreign key (product_id) references products(id),
  constraint fk_outbound_records_processed_by foreign key (processed_by) references users(id),
  constraint chk_outbound_records_total_quantity check (total_quantity > 0)
) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci;

create table outbound_record_items (
  id bigint unsigned not null auto_increment,
  outbound_record_id bigint unsigned not null,
  inventory_item_id bigint unsigned not null,
  pallet_id bigint unsigned not null,
  rack_slot_id bigint unsigned not null,
  quantity decimal(14,3) not null,
  fifo_rank int unsigned not null,
  primary key (id),
  key idx_outbound_record_items_record (outbound_record_id),
  key idx_outbound_record_items_inventory_item (inventory_item_id),
  constraint fk_outbound_record_items_record foreign key (outbound_record_id) references outbound_records(id) on delete cascade,
  constraint fk_outbound_record_items_inventory_item foreign key (inventory_item_id) references inventory_items(id),
  constraint fk_outbound_record_items_pallet foreign key (pallet_id) references pallets(id),
  constraint fk_outbound_record_items_rack_slot foreign key (rack_slot_id) references rack_slots(id),
  constraint chk_outbound_record_items_quantity check (quantity > 0),
  constraint chk_outbound_record_items_fifo_rank check (fifo_rank > 0)
) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci;
...
create table stock_movements (
  ...
  movement_type enum('inbound', 'outbound', 'move', 'adjustment') not null,
  ...
  created_at datetime(3) not null default current_timestamp(3),
  primary key (id),
  key idx_stock_movements_product_created_at (product_id, created_at),
  key idx_stock_movements_inventory_item (inventory_item_id),
  key idx_stock_movements_ref (ref_table, ref_id),
  constraint fk_stock_movements_inventory_item foreign key (inventory_item_id) references inventory_items(id),
  constraint fk_stock_movements_product foreign key (product_id) references products(id),
  constraint fk_stock_movements_pallet foreign key (pallet_id) references pallets(id),
  constraint fk_stock_movements_from_slot foreign key (from_rack_slot_id) references rack_slots(id),
  constraint fk_stock_movements_to_slot foreign key (to_rack_slot_id) references rack_slots(id),
...
5. `inbound_records`를 생성한다.
6. `stock_movements`에 `movement_type = 'inbound'`, `quantity_delta > 0`으로 기록한다.
...
6. `outbound_records`와 `outbound_record_items`를 생성한다.
7. 각 `inventory_items.quantity`를 차감한다.
8. `stock_movements`에 `movement_type = 'outbound'`, `quantity_delta < 0`으로 기록한다.
...
- 재고 이동은 `pallets.rack_slot_id`를 갱신하고 `stock_movements`에 `movement_type = 'move'`로 남긴다.
- 수량 조정은 `inventory_items.quantity` 직접 갱신과 함께 `stock_movements`에 `movement_type = 'adjustment'`로 남긴다.
